...
#define ISR_DEBUG_IO_MASK 0x0010
...
static void IRAM_ATTR timer_group_isr(void *para)
{
    ...
    /* Clear the interrupt */
    if (REG_GET_BIT(TIMG_INT_ST_TIMERS_REG(handle->config.tg_num) + handle->config.timer_num * 0x24, BIT(handle->config.timer_num))) {
        REG_SET_BIT(TIMG_INT_CLR_TIMERS_REG(handle->config.tg_num) + handle->config.timer_num * 0x24, BIT(handle->config.timer_num));
    }
    /* After the alarm has been triggered
        we need enable it again, so it is triggered the next time */
    REG_SET_BIT(TIMG_T0CONFIG_REG(handle->config.tg_num) + handle->config.timer_num * 0x24, TIMG_T0_ALARM_EN_M);
...
esp_err_t pwm_audio_start(void)
{
    esp_err_t res;
    pwm_audio_data_t *handle = g_pwm_audio_handle;
    pwm_audio_config_t *cfg = &handle->config;
    PWM_AUDIO_CHECK(NULL != handle, PWM_AUDIO_NOT_INITIALIZED, ESP_ERR_INVALID_STATE);
    ...
    REG_SET_BIT(TIMG_INT_ENA_TIMERS_REG(cfg->tg_num) + cfg->timer_num * 0x24, BIT(cfg->timer_num));
    REG_SET_BIT(TIMG_T0CONFIG_REG(cfg->tg_num) + cfg->timer_num * 0x24, TIMG_T0_EN_M);
    REG_SET_BIT(TIMG_T0CONFIG_REG(cfg->tg_num) + cfg->timer_num * 0x24, TIMG_T0_ALARM_EN_M);
#ifdef TIMG_T0_LEVEL_INT_EN_M
    REG_SET_BIT(TIMG_T0CONFIG_REG(cfg->tg_num) + cfg->timer_num * 0x24, TIMG_T0_LEVEL_INT_EN_M);
#endif
    portEXIT_CRITICAL_SAFE(&timer_spinlock);

    res = timer_start(cfg->tg_num, cfg->timer_num);
    return res;
}
...
esp_err_t pwm_audio_stop(void)
{
    ...
    portENTER_CRITICAL_SAFE(&timer_spinlock);
    REG_CLR_BIT(TIMG_INT_ENA_TIMERS_REG(cfg->tg_num) + cfg->timer_num * 0x24, BIT(handle->config.timer_num));
    REG_CLR_BIT(TIMG_T0CONFIG_REG(cfg->tg_num) + cfg->timer_num * 0x24, TIMG_T0_EN_M);
    REG_CLR_BIT(TIMG_T0CONFIG_REG(cfg->tg_num) + cfg->timer_num * 0x24, TIMG_T0_ALARM_EN_M);
#ifdef TIMG_T0_LEVEL_INT_EN_M
    REG_CLR_BIT(TIMG_T0CONFIG_REG(cfg->tg_num) + cfg->timer_num * 0x24, TIMG_T0_LEVEL_INT_EN_M);
#endif
    portEXIT_CRITICAL_SAFE(&timer_spinlock);
